	...
	return {
		...
		template:
					'<div class="ats-switch" ng-click="toggle()" ng-keypress="onKeyPress($event)" ng-class="{ \'disabled\': isDisabled }" role="switch" aria-checked="{{!!model}}">' +
						'<div class="switch-animate" ng-class="{\'switch-off\': !model, \'switch-on\': model}">' +
							'<span tooltip-append-to-body="true" tooltip="{{offTitle}}" style="background:#629FDA" class="switch-left" ></span>' +
							'<span class="knob"></span>' +
							'<span tooltip-append-to-body="true" tooltip="{{onTitle}}" style="background:#2DC7AC;color:#fff;"  class="switch-right"></span>' +
						'</div>' +
					'</div>',
		compile: function(element, attrs) {
			...
			return function postLink(scope, iElement, iAttrs, ngModel) {
				...
				scope.onTitle = attrs.onTitle;
				scope.offTitle = attrs.offTitle;
